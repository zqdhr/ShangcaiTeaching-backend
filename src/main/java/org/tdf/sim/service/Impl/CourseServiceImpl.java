...
public class CourseServiceImpl implements CourseService {
    ...
    @Autowired
    private CourseExperimentCategoryDao courseExperimentCategoryDao;
    ...
    public Pair<Boolean, String> saveCourse(AddCoursePost addCoursePost) {
        ...
        courseExperimentCategoryDao.saveAll(addCoursePost.categories.stream().map(x -> CourseExperimentCategoryEntity.builder()
                .type(2)
                .linkID(x)
                .categoryID(x).build()
        ).collect(Collectors.toList()));
    ...
    public List<CourseResp> getCourseHaveClasses(String ownerID, int type) {
        ...
        return courseDao.findAllByType(type).stream().filter(x -> courseClassDao.findAllByCourseID(x.getId()).stream().anyMatch(v -> v.getClassID().equals(optional.get().getClassID()))).map(x -> {
            List<CategoryEntity> categoryEntities = categoryDao.findAllCategoriesByLinked(x.getId());
